...
export function getDb(dbPath) {
  ...
  // Migration: ensure posted_messages has the per-channel UNIQUE constraint.
  // Older versions of the schema had UNIQUE(call_id) only, which broke
  // multi-guild posts and made ON CONFLICT(call_id, channel_id) fail.
  const pmSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='posted_messages'").get();
  if (pmSql && !pmSql.sql.includes('UNIQUE(call_id, channel_id)')) {
    db.exec(`
      BEGIN;
      CREATE TABLE posted_messages_new (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      INSERT INTO posted_messages_new (id, call_id, message_id, channel_id, posted_at)
        SELECT id, call_id, message_id, channel_id, posted_at FROM posted_messages;
      DROP TABLE posted_messages;
      ALTER TABLE posted_messages_new RENAME TO posted_messages;
